...
const ArticleItem: React.FC<ArticleType> = ({ date, title, description, link, tags }) => (
      ...
      <div className="md:col-span-3">
        ...
        <p className="mt-2 text-slate-600 dark:text-slate-400">
         ...
         <div className="mt-4 flex flex-wrap gap-2">
          {tags.map(tag => (
            <span key={tag} className="bg-slate-200/50 dark:bg-slate-800/50 text-slate-600 dark:text-slate-300 text-xs font-medium px-2.5 py-1 rounded-full">
              {tag}
            </span>
          ))}
        </div>
